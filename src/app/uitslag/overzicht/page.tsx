      ...
      <div className="min-h-screen">
        ...
        <div className="md:hidden">
          <div className="flex items-center justify-between px-4 pt-6 pb-3">
            ...
            <h1 className="font-serif font-semibold text-green">Uitslag overzicht</h1>
            <div className="w-12" />
          </div>
          <div className="px-5 pb-6 flex flex-col gap-2.5">
            {regels.length === 0 && <p className="text-sm text-inkSoft text-center mt-8">Nog geen uitslag geregistreerd.</p>}
            {regels.map((r) => (
              <div key={r.id} className="rounded-2xl border-[1.5px] border-line bg-white p-3.5">
                ...
                <div className="grid grid-cols-2 gap-y-1 text-xs">
                  ...
                  <div className="text-inkSoft mt-1">TYPE</div>
                  <div className="text-inkSoft mt-1">TARIEF</div>
                  <div className="font-medium capitalize">{r.uitslagType}</div>
                  <div className="font-medium">{r.tarief === "geetiketteerd" ? "Geëtiketteerd" : "Standaard"}</div>
                  <div className="text-inkSoft mt-1">UITSLAGTIJD</div>
                  <div className="text-inkSoft mt-1">AANTAL</div>
                  <div className="font-medium">{fmtDatumTijd(r.uitslagtijd)}</div>
                  <div className="font-medium">{r.aantalKazenUit} stuks</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="hidden md:block md:px-10 md:py-10 md:max-w-6xl">
          <div className="flex items-center justify-between mb-6">
            <h1 className="font-serif text-3xl font-semibold text-green">Uitslag overzicht</h1>
            <p className="text-sm text-inkSoft">{regels.length} regels (laatste 300)</p>
          </div>

          {regels.length === 0 ? (
            <p className="text-sm text-inkSoft">Nog geen uitslag geregistreerd.</p>
          ) : (
              ...
              <tbody>
                {regels.map((r) => (
                  <tr key={r.id} className="border-t border-line hover:bg-goldSoft/40">
                    ...
                    <td className="py-2.5 px-4 text-right">{r.aantalKazenUit}</td>
                    <td className="py-2.5 px-4 text-right font-semibold">{(r.nettoGramUit / 1000).toFixed(1)} kg</td>
                  </tr>
                ))}
              </tbody>
            </table>
